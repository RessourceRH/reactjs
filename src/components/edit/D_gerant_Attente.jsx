...
import M_Gerant from "./modification/M_Gerant";
import M_valide from "./modification/M_valide";

export default function D_gerant_Attente() {
  ...
  const fetchData = async () => {
    try {
       const counts = await axios.get(`${Lien}/script/gerant_attente`);
      setcount(counts.data.count)
      const response = await axios.get(`${Lien}/gerant/attente`);
...
const data = contrat.map((user) => user);
    ...
    },
    {
      ...
      },
      {
        ...
        cell: (user) => (
          ...
          <M_valide pers={user} onrefresh={handleRefresh}/>
        ),
        sortable: true,
      },
    {
        ...
        cell: (user) => (
          <div>
            ...
            <M_Gerant pers={user} onrefresh={handleRefresh}/>
  ...
  const generatePDF =async () => {
    ...
    doc.setFontSize(18);
    doc.text("Liste des gerants qui sont  en attentes", 14, yOffset+20);

    doc.save("Liste_des_gerants_qui_sont_en_attentes.pdf");
  };
 
  return (
    
              <div className="card-body">
                <h4 className="card-title" style={{color:'black'}}>LISTE DES GERANTS EN ATTENTES</h4>
