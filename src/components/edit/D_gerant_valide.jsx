...
import M_annule from "./modification/M_annule";

export default function D_gerant_valide() {
  ...
  const fetchData = async () => {
    try {
     const counts = await axios.get(`${Lien}/script/gerant_valide`);
     setcount(counts.data.count)
      const response = await axios.get(`${Lien}/gerant/valide`);
...
const data = contrat.map((user) => user);
    ...
    },
    {
      ...
      },
      {
        ...
        cell: (user) => (
          ...
          <M_annule pers={user} onrefresh={handleRefresh}/>
        ),
        sortable: true,
      },
    {
  ...
  const generatePDF = async() => {
    ...
    doc.setFontSize(18);
    doc.text("Liste des gerants qui sont  en validés", 14, yOffset+20);

    doc.save("Liste_des_gerants_qui_sont_validés.pdf");
  };
 
  return (
    
              <div className="card-body">
                <h4 className="card-title" style={{color:'black'}}>LISTE DES GERANTS VALIDES</h4>
